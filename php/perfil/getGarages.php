...
/**
 * @Author: erikfer94
 * @Date:   2018-10-09 11:37:12
 * @Last Modified by:   erikfer94
 * @Last Modified time: 2018-12-27 15:41:32
 */
session_start();
...
if(!isset($_SESSION["iduser"])) {
	$garages = $Garage -> colaboratingGarageByTen($cuenta,$_POST["t"],false);
	$owner = false;
}
else{
...
{		
	...
	if(isset($_SESSION["iduser"]) && !$owner)
	{
	...
	if($owner || ((isset($garageInfo["privacidad"])) ? $garageInfo["privacidad"] : 1)!=3 || (!empty($followingGarage) && $followingGarage["aceptado"]==1))
	{
		...
		elseif (isset($_SESSION["iduser"]) && $_SESSION["iduser"]==0 && $cuenta!=$garage["userOwner"]) {
			continue;
		}
		else 
		{ ?>
		    			...
		    			<div class="head-info">
				    		...
				    		<div class="time">
				    			...
				    			<ul id="ulgarage" class="icon-bar">
				    				<?php if($cuenta!=$garage["userOwner"]){ 
				    					if($owner){?>
				    				...
				    				<?php }
				    					...
				    					<li class="pointer">
					    					<span class="colaboratingTag">Colaborador</span>
					    				</li>
				    				<?php }
				    				}
				    				else{ ?>
				    					<li class="pointer">
					    					<span class="ownerTag">Propietario</span>
					    				</li>
				    				<?php 
			    					}
			    					...
			    					if(!$owner)
			    					{
				    					if(!$Like->alreadyLike(isset($_SESSION["iduser"]) ? $_SESSION["iduser"] : 0, 2, $garage["idAccount"])){ ?>
